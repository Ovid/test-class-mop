...
role Test::Class::MOP::Role::AutoUse {
    ...
    has $!class_name is ro = do {
        # we use a do{} block to keep this private
        my $name = $_->get_class_name_to_use or return;
    ...
    method get_class_name_to_use {
        my $name = mop::meta($self)->name;
        $name =~ s/^[^:]+:://;
        return $name;
    }
}
...
With L<Test::Class::MOP::Role::AutoUse>, you only rename the test class name
...
Your test subclass might look like this:
 ...
 use Test::Class::MOP extends => "TestsFor::Person";
...
L<Test::Class::MOP::Role::AutoUse> role doesn't hardcode the classname (at
